...
//   - COALESCE (leading-edge): a lone ping reconciles immediately; followers
//     within a short window batch into ONE pass, so a 50-row bulk write costs
//     one round trip, not fifty — without taxing single changes.
//   - DELTA-APPLY: `{op: DELETE, id}` removes locally with no fetch at all;
//     a ding carrying the full `row` (the private doorbell's normal case)
//     applies directly with ZERO fetches; only id-only dings (RLS-on tables
//     without owner_id, legacy public pings) fetch the changed rows by id
//     through the gateway (full auth + RLS) and upsert them.
...
export class LiveEntityStore<T extends EntityRow = EntityRow> {
  ...
  /** When the last reconcile pass ran — drives leading-edge coalescing. */
  ...
  private onDing(p: BoolChangePayload): void {
    this.queue.push(p);
    if (this.flushTimer === null) {
      // LEADING-EDGE coalescing: a lone change (the common case) reconciles on
      // the next macrotask — no felt delay — while a burst still collapses:
      // once a pass has run, followers within the window batch into one more
      // pass. A pure trailing window taxed every single change 50ms just in
      // case a burst was coming.
      const sinceLast = Date.now() - this.lastFlushAt;
      const delay = sinceLast >= COALESCE_MS ? 0 : COALESCE_MS - sinceLast;
